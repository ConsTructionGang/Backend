		...
		if(!results[0].isSupplier) {
			...
			helpers.runQuery(query.authorIsSupplier(request.payload), function(results) {
				if(results[0].isSupplier) {
					...
					helpers.runQuery(query.alreadyReviewed(request.payload, request.params), function(result) {
						if (result.length) {
							return reply({
...
function insertReview(request, reply) {
	helpers.runQuery(query.post(request), function(results, err) {
		if (err) {
			return reply({
...
function retrieveAll(request, reply) {
	helpers.runQuery(query.isSupplier(request.params), function(results) {
		if(results.length == 0 || !results[0].isSupplier){
			...
			helpers.runQuery(query.retrieve(request.params), function(results) {
				return reply({
					results
				}).code(200);
			});
...
function remove(request, reply) {
	helpers.runQuery(query.remove(request.payload, request.params), function(results, err) {
		if (err) {
			return reply({
